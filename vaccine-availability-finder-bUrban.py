...
logging.basicConfig(filename='myappbUrban-prod.log', format='%(asctime)s : %(levelname)s - %(message)s', level=logging.INFO,
...
def saveGlobalListState():
    ...
    if save_state_timer == 7200:
        outputFile = open('global_list_bUrban.dat', 'wb')
...
def retrieveGlobalListState():
    global centerList_Global
    inputFile = open('global_list_bUrban.dat', 'rb')
...
schedule.every(30).seconds.do(cowinApiCall, args.district_id, args.age, args.chatId)
